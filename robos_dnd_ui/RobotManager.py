...
actionOptions = [MotorAction(), BodyTurnAction(), HeadTurnAction(), HeadTiltAction(), PauseAction()]
...
def onActionClicked(event):
	...
	print(int(event.widget.cget("text").split("\n")[0].split(" ")[1]))
...
def run_animation():
    ...
    for action in actions:
        if action is not None:
            duration += action.properties.get('Duration', (0, 0, 0))[0]/1000
    print(duration)
    sm = GifFrame()
